...
export default function Carrer() {
    ...
    const populateBlogList = () => {
        const getData = async () => {
            const response = await ApiClient.call(ApiClient.REQUEST_METHOD.POST, '/career/getAllActiveCareer', {}, {}, {Cookie: ApiClient.cookie, Authorization: ApiClient.authorization}, false);
                            ...
                            <div className="job-list" >
                                <div className="row">
                                    ...
                                    {(allActiveCareer || []).map((career, i) => {
                                        const {_id, degination, department, vacancy, experiance, location} = career;

                                        return (<div className=" job_inner wow fadeInUp" >
                                            <Grid item xs={9}>
                                                <h3 className="job-title">{degination} - {department} - {vacancy || 0} Position</h3>
                                                <div className="job-details-section">
                                                    <div className="job-detail">
                                                        <span className="detail-value"> <DateRangeIcon style={{color: "#FF7601", }} />{experiance}</span>
                                                    </div>
                                                    <div className="job-detail">
                                                        <span className="detail-value"><LocationOnIcon style={{color: "#FF7601", }} /> {location}</span>
                                                    </div>
                                                </div>
                                            </Grid>
                                            <Grid item xs={3}>
