...
function deleteServiceEnquiry(Models) {
    async function deleteFun(req, res) {
        try {
            ...
            let deleteData = await Models.ServiceEnquiryDB.remove({ _id: req.body._id });
            console.log('deleteData is', deleteData)
            if (deleteData) {
                // if data found check verified or not
                res.send({ status: true, message: "Service Enquiry Deleted Successfully" });
            } else {
...
module.exports = deleteServiceEnquiry;